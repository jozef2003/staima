"""
Nora OAuth Flow — einmalig lokal ausführen.
...
BOT_EMAIL = "nora@myty.agency"  # wird nach Login mit echter Email überschrieben


def main():
    print("=== Nora Google OAuth Flow ===")
    print("Browser öffnet sich — mit Olivers Google Account einloggen.\n")

    flow = InstalledAppFlow.from_client_config(CLIENT_CONFIG, SCOPES)
    creds = flow.run_local_server(port=0, prompt="consent", access_type="offline")

    if not creds.refresh_token:
        print("FEHLER: Kein refresh_token erhalten. Bitte nochmal versuchen.")
        sys.exit(1)

    expiry = creds.expiry.replace(tzinfo=timezone.utc).isoformat().replace("+00:00", "Z") if creds.expiry else None

    # ── 1. authorized_user.json ───────────────────────────────
    authorized_user = {
    ...
    print(f"   scp bots/nora/authorized_user.json root@159.69.221.169:/opt/nora/.openclaw/")
    print(f"   scp bots/nora/gogcli_token.json root@159.69.221.169:/tmp/nora_token.json")
    print("\n2. Token in gogcli importieren:")
    print(f"   ssh root@159.69.221.169 'HOME=/opt/nora gog auth tokens import /tmp/nora_token.json'")
    print(f"   ssh root@159.69.221.169 'rm /tmp/nora_token.json'")
    print("\n3. Nora neustarten:")
    print(f"   ssh root@159.69.221.169 'systemctl restart openclaw-nora'")
